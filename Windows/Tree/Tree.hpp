...
namespace aisa {
   ...
   public:
       ...
       */
      inline typename Tree<T>::PreOrderIterator preBegin() const;
       ...
       */
      inline typename Tree<T>::PreOrderIterator preEnd() const;
       ...
       */
      inline typename Tree<T>::PostOrderIterator postBegin() const;
       ...
       * @return 'PostOrderIterator' to the root node.
       */
      inline typename Tree<T>::PostOrderIterator postEnd() const;
       ...
       */
      typename Tree<T>::PreOrderIterator pushFrontChild(const TreeIterator<T>& parent, const T& data);
       ...
       * @throws std::bad_alloc Thrown if memory allocation for the children node fails.
       */
      typename Tree<T>::PreOrderIterator pushBackChild(const TreeIterator<T>& parent, const T& data);
       ...
       * @throws std::bad_alloc Thrown if memory allocation for the children node fails.
       */
      typename Tree<T>::PreOrderIterator insertChild(const TreeIterator<T>& parent, const TreeIterator<T>& childNode, const T& data);
       ...
       */
      void erase(TreeIterator<T>& node);
   ...
   private:
       ...
       * @throws std::bad_alloc Thrown if memory allocation fails when copying. 
       */
      void clone(const Tree<T>& source);
   ...
   template <class T>
   Tree<T>::Tree(const Tree<T>& source) : _root(NULL) {
      try {
         clone(source);
      }
      catch(std::bad_alloc& ex) {
   ...
   template <class T>
   void Tree<T>::setRoot(const T& data) {
      if(_root == NULL) {
         try {
            _root = new TreeNode<T>(data);
         }
   ...
   template <class T>
   typename Tree<T>::PreOrderIterator Tree<T>::pushFrontChild(const TreeIterator<T>& parent, const T& data) {
      TreeNode<T>* child;
      try {
   ...
   template <class T>
   typename Tree<T>::PreOrderIterator Tree<T>::pushBackChild(const TreeIterator<T>& parent, const T& data) {
      TreeNode<T>* child;
      try {
         child = new TreeNode<T>(data, parent._pointer);
      }
   ...
   template <class T>
   void Tree<T>::clone(const Tree<T>& source) {
   ...
   private:
      ...
      friend PreOrderIterator Tree<T>::pushBackChild(const TreeIterator<T>& parent, const T& data);
      friend PreOrderIterator Tree<T>::pushFrontChild(const TreeIterator<T>& parent, const T& data);
      friend PreOrderIterator Tree<T>::insertChild(const TreeIterator<T>& parent, const TreeIterator<T>& childNode, const T& data);
    ...
    * also lets the user navigate through descendants and ancestors in a secuential fashion.
    *
    * IMPORTANT: The use of post-order iterators can be a little tricky at first.
    * You have to understand that once you assign another post-order iterator to
    * your post-order iterator (or copy-construct it), your post-order iterator will
    * automatically point to the last element of that tree/sub-tree (and not to the
    * element that the post-order iterator you copied is actually pointing to).
    *
    * @author Francisco Aisa García
    * @version 0.2
    */
   ...
   typename Tree<T>::PostOrderIterator& Tree<T>::PostOrderIterator::operator=(const PostOrderIterator& rhs) {
      if(this != &rhs) {
	     // It makes a copy of the pointer of the rhs
         TreeIterator<T>::operator=(rhs);
   ...
   typename Tree<T>::PostOrderIterator& Tree<T>::PostOrderIterator::operator++() {
      ...
      // Keep traveling the stack until you find a leaf
      while(!_pathStack.empty()) {
         std::pair<TreeNode<T>*, typename std::list< TreeNode<T>* >::iterator> topNode = _pathStack.top();

         // If the current node has a children that hasn't been explored
	     // search in that branch until you find a leaf
         while(topNode.second != topNode.first->_children.end()) {
            it = topNode.second;
            _pathStack.top() = std::pair<TreeNode<T>*, typename std::list< TreeNode<T>* >::iterator>(topNode.first, ++it);
      ...
      // Reached the end of the tree/sub-tree
      TreeIterator<T>::setPointer(NULL);
      return *this;
   }
